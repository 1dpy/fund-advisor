...
- **五个标签页**：
  ...
  5. **实时赛道榜**：当日实时赛道综合得分 Top 10 —— `src/realtime_quotes.js` 抓**ETF 真实成交价 + 动量**合成综合分（持续性 75% + 当日 ETF 实时 25%，少追脉冲），横向柱图（红涨绿跌）+ 明细表；离线/沙箱自动回退确定性合成分，复试可现场切 `实盘净值` 看真实盘中强弱。
...
### 数据源口径说明（已锁定 · 2026-07-22）

用户授权"哪种数据最准、最能赚钱就用哪种"。据此锁定以下三层数据架构：

| 层 | 数据 | 来源 | 用途 | 谁取 |
|---|---|---|---|---|
| ① 实时赛道强度 | 赛道 ETF **真实成交价**（秒级） | 腾讯 `qt.gtimg.cn`（主）+ 东财 `push2`（备） | 实时赛道榜 + 动态选基的"活信号" | agent 直连（沙箱/本机均可，已实测可用） |
| ② 因子训练 | 各基金**真实单位净值 NAV**（T+1） | 东方财富历史净值 | walk-forward 自我迭代 → 元参数（topK/权重） | agent 每日自迭代 |
| ③ 单基盘中估值 | 场外 C 类基金**盘中估值** | 同花顺/东财（需本机 `npm run live` 桥接） | 精确看每只持仓当日盈亏（匹配同花顺 App） | 用户本机 → 写 `data/live_snapshot.json` |
...
| 数据源 | 腾讯 / 东财（赛道 ETF 实时成交价，主实时源）、东方财富（历史净值 + 自我迭代训练）、同花顺 / 东财（场外基金盘中估值，需本机 `npm run live` 桥接） |
...
概览页底部「📷 上传持仓截图更新」卡片：选图 / 拖入支付宝·天天基金截图 → 后端 OCR（tesseract.js，可选）识别**基金名 / 持有金额 / 持有收益** → 可编辑预览 → 点「核对无误, 更新持仓」写入 `holdings.json`。

- **识别仅供参考，必须逐行核对**（防 OCR 错位产生假持仓）；不准可直接把截图发 AI 助手（读图更准）。
- **启用 OCR**：`npm run ui:ocr`（首次自动装 `tesseract.js` 并下载中文语言包）。不装则界面临时回退提示，不阻断其他功能。
- 写入时**自动保留锁定约定**：广发全球精选 `021277` 压舱石、易方达信息产业 `019018` 减仓后持有；成本反推 = 现值 − 持有收益。
- 配套接口：`POST /api/holding-image`（存图 + OCR + 候选）、`POST /api/holding-confirm`（核对后写账）。
